...
@Component({
  selector: 'app-shop-store-message-main',
...
export class StoreMessageMainComponent implements OnInit {
  ...
  submitCustomerService() {
    ...
    let lineId = this.activeRoute.snapshot.params['lineId'];
    let options = {
      lineId,
      context: this.replyMessage
    };
    this.storeMessage.submitIssue(options).then((data) => {
  ...
  requestCloseMessage() {
    ...
    let lineId = this.activeRoute.snapshot.params['lineId'];
    this.storeMessage.requestCloseMessage({
      lineId
    }).then((data) => {
      this.status = 'Closed';
      this.judgeMessageIsClose();
