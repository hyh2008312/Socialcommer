...
@Component({
  selector: 'app-shop-cart-pay',
  templateUrl: './shop-cart-pay.component.html',
  styleUrls: ['../_shop-cart.scss']
})

export class ShopCartPayComponent implements OnInit{
  ...
  changeShippingState($event) {
    let cid = $event;
    this.shopCartService.getStateList({
      cid
    }).then((data)=> {
      this.states = data;
    })
  ...
  continue(mobile?:any) {
    ...
    let self = this;
    self.shopCartService.createShippingAddress(stepOneObject).then((data) => {
      ...
      if(stepOneObject.isSaveAddress) {
        ...
        self.shopCartService.addShippingAddress(shippingAddress);
      } else {
  ...
  fieldRequired() {
    let dialogRef = this.dialog.open(ShopRequireDialogComponent, {
      data: {}
    });
  ...
  stockRequired() {
    let dialogRef = this.dialog.open(ShopErrorDialogComponent, {
      data: {},
      disableClose: true
    });
